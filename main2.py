...
class EntrAI:
    ...
    def handle_user_type(self, key):
        ...
        # Handle commands
        if "!stop" in self.user_type_buffer:
            print("stop")
            self.writing = False
            self.text_buffer.clear()
            self.user_type_buffer = ""
            return False
        elif "!cpresp" in self.user_type_buffer:
        ...
        if not self.writing: return True
        ...
        if key.event_type != "down":
            next_char = self.text_buffer.pop(-1)
        else:
            next_char = ""
        ...
        keyboard.write(next_char, delay=0.2)
